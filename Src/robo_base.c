...
//
//--------------------------------------------------------------------------------------------------//
void PID_Speed_Cal(Speed_System* Speed_Motor,uint8_t *Tx_Data)
{
//`	if(Speed_Motor->Protect.State!=WORKING) return ;
	...
	Tx_Data[0]=((int16_t)Speed_Motor->Speed_PID.output)>>8;Tx_Data[1]=(int16_t)Speed_Motor->Speed_PID.output;
}
